...
isValidClause clause = any (\literal -> complement literal `Set.member` clause) clause
...
-- pentru toate clauzele, e ok daca are ca membru orice element din interpretare
...
interpretations = Set.foldl (\acc v -> [x | lit <- [v, -v], s <- acc, let x = Set.insert lit s]) [Set.empty]
-- se creaza un nou set de seturi, astfel: pt fiecare set partial creat, se iau variabilele v si -v si se adauga in ele,
-- iar rezultatul partial se adauga inapoi in acc prin foldl
...
RĂSPUNS: 
1. Atunci cand gasim o interpretare care satisface formula, ne oprim din cautare  => fara testari si generari reduntante
Procesul arata astfel: se genereaza prima interpretare, daca satisface, ne oprim, altfel generam urmatoarea interpretare
si continuam procesul.

2. Deoarece Set respecta un set de constrangeri (sa aiba elementele in ordine, sa nu aibe duplicate), el trebuie sa vada toate
elementele sale. Astfel, s-ar fi generat toate interpretarile, si avantajul folosirii lui any s-ar fi anulat. 
-}
isSatisfiable :: Formula -> Bool
isSatisfiable formula = any (`satisfiesFormula` formula) (interpretations (formulaVariables formula))
-- satisfiesFormula aplicat cu garzi pt ca e point free
-- pt fiecare interpretare, se cauta una care sa satisfaca formula
