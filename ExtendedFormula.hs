...
eliminate literal =
    Map.mapKeys (complement literal `Set.delete`) .
    Map.filterWithKey (const $ Set.notMember literal)
...
firstPureLiteral formula =
    Set.foldr combine Nothing $
    promote formulaLiterals formula
  where
    combine literal acc
        | promote (isPureLiteral literal) formula = Just literal
        | otherwise = acc
...
firstUnitClause = Map.foldrWithKey combine Nothing
  where
    combine clause originalClause acc
        | isUnitClause clause = Just (Set.findMin clause, originalClause)
        | otherwise = acc
...
process :: Bool
        ...
        -> History
        -> History
process repeated supplier constructor = go
  where
    go history@((_, formula) : _) = maybe history func $ supplier formula
      where
        func =
            (if repeated then go else id) . 
            (: history) . 
            (id &&& ((`eliminate` formula) . getLiteral)) .
            constructor
