...
resolve literal clause1 clause2
  -- reuniunea celor doua clauze minus literalul si complementul sau
  | Set.member (complement literal) clause2 = let union = Set.union clause1 clause2 
                                          in Set.delete (complement literal) (Set.delete literal union)
  | otherwise = clause2
...
learn = foldl (\acc x -> case x of
                        Unit literal clause -> resolve literal clause acc
                        _ -> acc)
-- acumulatorul initial e clauza
-- daca actiunea e de tip Unit, se ia literalul si se rezolva cu clauza
-- daca nu, se returneaza clauza curenta
...
satisfy formula = func 1 [(NOP, extendFormula formula)]
  where 
    emptyClause = Set.empty
    
    func :: Int -> History -> (Maybe Interpretation, History)
    func step history = 
      case step of
        -- 1. se prelucreaza toate clauzele unitare
        1 -> let newHistory = processUnitClauses history
             in func 2 newHistory
        
        -- 2. daca formula devine vida, e satisfiabila => construim interpretarea
        2 | Map.null (snd $ head history) ->
            (Just $ getInterpretation history, history)

        -- daca formula contine clauza vida (conflict), trebuie sa invatam o noua clauza
          | isJust emptyClauseValue ->
            let originalEmptyClause = fromMaybe Set.empty emptyClauseValue     -- extrage valoarea fara maybe
                learnedClause = learn originalEmptyClause (map fst history)    -- learn cu toate actiunile din history
            in if Set.null learnedClause          -- daca e null, e nesatisfiabila
               then (Nothing, history)
               else let backHistory = backtrackToUnitClause learnedClause history   -- backtrack pana in cel mai distant punct in care clauza e unitara
                    in func 1 backHistory
            
          | otherwise -> 
            func 3 history
          where
            emptyClauseValue = Map.lookup Set.empty (snd $ head history)      -- rezultatul e cu maybe
        
        -- seprelucreaza toti literalii puri, dar daca nu exista, se asuma cel mai mic literal si se revine la 1
        3 -> let newHistory = processPureLiterals history
                 changed = newHistory /= history
             in if changed
                then func 1 newHistory
                else func 4 history
        
        4 -> let newHistory = decide history    -- se asuma cel mai mic literal
             in func 1 newHistory
    
    -- Extract interpretation from history
    getInterpretation :: History -> Interpretation
    getInterpretation history
      | null history = Set.empty
      | Unit l _ <- action = Set.insert l $ getInterpretation $ tail history
      | Pure l <- action = Set.insert l $ getInterpretation $ tail history
      | Decide l <- action = Set.insert l $ getInterpretation $ tail history
      | otherwise = getInterpretation $ tail history
      where
           action = fst $ head history
...
-}
    ...
    encode problem = let nodes_set = nodes (graph problem)
                         edges_set = edges (graph problem)
                         emptySet = Set.empty
                         -- fiecare nod cu cele 3 culori
                         nodesWithColors = Set.foldl (\acc n ->  [n * 10 + 3, n * 10 + 2, n * 10 + 1] : acc) [] nodes_set               -- conditia de cel putin o culoare
                         pairs_of_2 = foldl (\acc nodes ->[[complement x, complement y] | x <- nodes, y <- nodes, x /= y] ++ acc) [] nodesWithColors   -- conditia de cel mult o culoare
                         dif_colors = foldl (\acc (x, y) -> [[complement (x * 10 + 1), complement (y * 10 + 1)], 
                                                             [complement (x * 10 + 2), complement (y * 10 + 2)], 
                                                             [complement (x * 10 + 3), complement (y * 10 + 3)]] ++ acc) [] edges_set   -- conditia de capete de culori dif
                    in
                        toFormula (pairs_of_2 ++ dif_colors ++ nodesWithColors) -- toate clauzele            
...
-}
    ...
    decode interpretation problem = let literals = Set.filter isPositive interpretation   -- ultimii literali, cei pozitivi
                                        solution = foldl (\acc x -> Map.insert (x `div` 10) (color (x `mod` 10)) acc) Map.empty literals
                                    in problem { coloring = solution }
                                    where
                                        color 1 = Red
                                        color 2 = Green
                                        color 3 = Blue
